...
public class ReplyToChildrenGroupPricing extends AbstractReply {
    public ReplyToChildrenGroupPricing(SilentSender sender, Map<Long, UserState> chatStates) {
    ...
    public void reply(Long chatId, Message message) {
        switch (message.getText()) {
            case "Light" -> promptWithKeyboardForState(chatId, Pricing.ChildrenLightPackage,
                    KeyboardFactory.getPackageKeyboard(), ADULTS_GROUP_PRICING);
            case "Favourite" -> promptWithKeyboardForState(chatId, Pricing.ChildrenFavouritePackage,
                    KeyboardFactory.getPackageKeyboard(), ADULTS_GROUP_PRICING);
            case "Delicious" -> promptWithKeyboardForState(chatId, Pricing.ChildrenDeliciousPackage,
                    KeyboardFactory.getPackageKeyboard(), ADULTS_GROUP_PRICING);
